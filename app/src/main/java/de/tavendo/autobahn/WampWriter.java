...
/**
 * Autobahn WAMP writer, the transmitting leg of a WAMP connection. This writer
 * serializes WAMP messages forwarded from the foreground thread (the master) to
 * this object running on the writer thread. WAMP messages are serialized to
 * JSON, and then sent via WebSockets.
 */
public class WampWriter extends WebSocketWriter {
	...
	protected void processAppMessage(Object msg) throws WebSocketException, IOException {
		...
		JsonGenerator generator = this.mJsonFactory.createJsonGenerator(this.mPayload);

		try {

			// serialize WAMP messages to JSON: the code here needs to
			// understand
			// any client-to-server WAMP messages forward from the foreground
			// thread
		...
		this.sendFrame(1, true, this.mPayload.getByteArray(), 0, this.mPayload.size());
