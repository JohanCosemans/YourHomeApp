...
public class WampConnection extends WebSocketConnection implements Wamp {
	...
	protected void createWriter() {
		...
		if (WampConnection.DEBUG) {
			Log.d(WampConnection.TAG, "writer created and started");
		}
	}
	...
	protected void createReader() {
		...
		if (WampConnection.DEBUG) {
			Log.d(WampConnection.TAG, "reader created and started");
		}
	}
	...
	private String newId(int len) {
		...
		for (int i = 0; i < len; i++) {
			buffer[i] = WampConnection.mBase64Chars[this.mRng.nextInt(WampConnection.mBase64Chars.length)];
		}
	...
	public void connect(String wsUri, Wamp.ConnectionHandler sessionHandler) {
		...
		this.connect(wsUri, sessionHandler, options, null);
	}
	...
	public void connect(String wsUri, Wamp.ConnectionHandler sessionHandler, WampOptions options, List<BasicNameValuePair> headers) {
		...
		try {
			this.connect(wsUri, new String[] { "wamp" }, new WebSocketConnectionHandler() {
				...
				public void onClose(int code, String reason) {
					if (WampConnection.this.mSessionHandler != null) {
						WampConnection.this.mSessionHandler.onClose(code, reason);
					} else {
						if (WampConnection.DEBUG) {
			...
			if (this.mSessionHandler != null) {
				this.mSessionHandler.onClose(WebSocket.ConnectionHandler.CLOSE_CANNOT_CONNECT, "cannot connect (" + e.toString() + ")");
			} else {
				if (WampConnection.DEBUG) {
					Log.d(WampConnection.TAG, "could not call onClose() .. handler already NULL");
				}
	...
	public void connect(String wsUri, Wamp.ConnectionHandler sessionHandler, List<BasicNameValuePair> headers) {
		...
		this.connect(wsUri, sessionHandler, options, headers);
	}
	...
	protected void processAppMessage(Object message) {
		...
		if (message instanceof WampMessage.CallResult) {
			...
			if (this.mCalls.containsKey(callresult.mCallId)) {
				CallMeta meta = this.mCalls.get(callresult.mCallId);
			...
			if (this.mCalls.containsKey(callerror.mCallId)) {
				CallMeta meta = this.mCalls.get(callerror.mCallId);
				if (meta.mResultHandler != null) {
			...
			if (this.mSubs.containsKey(event.mTopicUri)) {
				SubMeta meta = this.mSubs.get(event.mTopicUri);
			...
			if (WampConnection.DEBUG) {
				Log.d(WampConnection.TAG, "WAMP session " + welcome.mSessionId + " established (protocol version " + welcome.mProtocolVersion + ", server " + welcome.mServerIdent + ")");
			}

		} else {

			if (WampConnection.DEBUG) {
				Log.d(WampConnection.TAG, "unknown WAMP message in AutobahnConnection.processAppMessage");
			}
		}
	}
	...
	private void call(String procUri, CallMeta resultMeta, Object... arguments) {
		...
		WampMessage.Call call = new WampMessage.Call(this.newId(), procUri, arguments.length);
		for (int i = 0; i < arguments.length; ++i) {
			call.mArgs[i] = arguments[i];
		}
		this.mCalls.put(call.mCallId, resultMeta);
	...
	public void call(String procUri, Class<?> resultType, CallHandler resultHandler, Object... arguments) {
		...
		this.call(procUri, new CallMeta(resultHandler, resultType), arguments);
	}
	...
	private void subscribe(String topicUri, SubMeta meta) {
		...
		String uri = this.mOutgoingPrefixes.resolveOrPass(topicUri);

		if (!this.mSubs.containsKey(uri)) {

			this.mSubs.put(uri, meta);

			WampMessage.Subscribe msg = new WampMessage.Subscribe(this.mOutgoingPrefixes.shrink(topicUri));
			this.mWriter.forward(msg);
		}
	}
	...
	public void subscribe(String topicUri, Class<?> eventType, EventHandler eventHandler) {
		...
		this.subscribe(topicUri, new SubMeta(eventHandler, eventType));
	}
	...
	public void unsubscribe() {
		...
		for (String topicUri : this.mSubs.keySet()) {
	...
	public void prefix(String prefix, String uri) {
		...
		String currUri = this.mOutgoingPrefixes.get(prefix);

		if (currUri == null || !currUri.equals(uri)) {

			this.mOutgoingPrefixes.set(prefix, uri);
	...
	public void publish(String topicUri, Object event) {
		...
		WampMessage.Publish msg = new WampMessage.Publish(this.mOutgoingPrefixes.shrink(topicUri), event);
		this.mWriter.forward(msg);
	}
}
