...
/**
 * Mapping between CURIEs and URIs. Provides a two-way mapping between CURIEs
 * (Compact URI Expressions) and full URIs.
 *
 * \see http://www.w3.org/TR/curie/
 *
 * \todo Prefixes MUST be NCNames
...
public class PrefixMap {
	 ...
	 *            Prefix to be mapped.
	 * @param uri
	 *            URI the prefix is to be mapped to.
	 */
	...
	public String get(String prefix) {
		return this.mPrefixes.get(prefix);
	}
	 ...
	 *
	 * @param prefix
	 *            Prefix for which mapping should be removed.
	 * @return The URI the prefix was mapped to (when removed), or null when
	 *         prefix is unmapped (so there wasn't anything to remove).
	 */
	public String remove(String prefix) {
		if (this.mPrefixes.containsKey(prefix)) {
			String uri = this.mPrefixes.get(prefix);
	...
	public String resolve(String curie) {
		...
		if (i > 0) {
			...
			if (this.mPrefixes.containsKey(prefix)) {
				return this.mPrefixes.get(prefix) + curie.substring(i + 1);
			}
		}
		return null;
	}
